...
void CreateDArray(DArray *DArrayHead, unsigned int InitialSize) {
  ...
  ************************************************************************/
	DArrayHead -> Capacity = InitialSize;//set capacity to initial size
	DArrayHead -> EntriesUsed = 0;	//set entries used to 0
	if (InitialSize < 0) {	//if the size wanted is less than 0
		fprintf(stderr,"No allocation needed");
		exit(6);	
	}
	else{
		...
		// check if pointer to payload isn't null
		}
		else{	// if pointer to payload is null
...
unsigned int PushToDArray(DArray *DArrayHead, Data *PayloadPtr)
{
   ...
   ************************************************************************/
	if(DArrayHead -> EntriesUsed >= DArrayHead -> Capacity){
		...
		DArrayHead -> Payload = (Data*)realloc(DArrayHead ->\
 Payload,(DArrayHead -> Capacity)*sizeof(Data));
		if(DArrayHead -> Payload == NULL){	//if the pointer to ptr is null
		fprintf(stderr,"Memory re-allocation unsuccessful");
		exit(-101);
		}
		else{
		memcpy(&(DArrayHead -> Payload[DArrayHead ->\
 EntriesUsed]),PayloadPtr,sizeof(Data)); 
		}
	}
	//increases the entries used by 1
	memcpy(&(DArrayHead -> Payload[DArrayHead ->\
 EntriesUsed]),PayloadPtr,sizeof(Data));
	DArrayHead -> EntriesUsed ++; //
	return DArrayHead -> EntriesUsed - 1;	
	//returns the array index of the last element inserted
 ...
 *************************************************************************/
	DArrayHead -> EntriesUsed = 0; //Entries used is set to 0
	DArrayHead -> Capacity = 0;	// capcity is set to zero
	free(DArrayHead -> Payload);	//de-allocate memory 
