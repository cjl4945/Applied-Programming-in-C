...
int main(int argc, char *argv[])
 ...
 /* Declare your data and calculation variables here */
...
#ifdef EN_TIME
   printf("Loop is currently being unrolled by a factor of Two\n");
   ...
   if (1 != argc)
    ...
    //fprintf(stderr, "Loop is currently being unrolled by a factor of Two\n");
   ...
   /* Insert end calc timing */
    ...
    #ifdef EN_TIME
    PRINT_TIMER(data);
    PRINT_RTIMER(data,100);
    PRINT_TIMER(calculation);
    PRINT_RTIMER(calculation,20000);
    #endif
   ...
   for (lcv=0; lcv < DataSet->NextElement; lcv+= 2)
    {
      ...
      S_XX2 += DataSet->Data_X[lcv+1] * DataSet->Data_X[lcv+1];
      S_XY2 += DataSet->Data_X[lcv+1] * DataSet->Data_Y[lcv+1];
      S_X2  += DataSet->Data_X[lcv+1];
      S_Y2  += DataSet->Data_Y[lcv+1];
    } /* for() */
