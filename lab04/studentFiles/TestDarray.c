/******************************************************************************
 * TestDarray_hw.c - skeleton of code to test DynamicArray module - student file
 *
 ...
 * R. Repka  10/04/2018 - Close file
 * R. Repka  06/14/2019 - Simplified code
 * R. Repka  08/10/2019 - Provided text read code
 ******************************************************************************/
#include <stdio.h>
#include <string.h>
   ...
   if (2 == argc) /* note that argc 2 means one argument given */
     {
      ...
      if (NULL != DataFile)
        ...
        /********************************************************************
         ...
         Read all Data from text file   use ReadDat() provided 
         lose the Data file - use fclose(DataFile)
	...
	// reads data from file and puts them into the array
        int i;
	
        for (i = 0; i<6; i++)
        {
	  printf("%d ", i+1);  //line of element 
          printf(TestDynamicArray.Payload[i].String);// index of the string 
          printf("\n");
	}
        int k;
        for (k = TestDynamicArray.EntriesUsed - 6; k < TestDynamicArray.EntriesUsed; k++)
        {
	  printf("%d ", k+1);//index of element
          printf(TestDynamicArray.Payload[k].String);//current element
	  printf("\n");
        }
        ...
        /* Be sure to close the file or you get a memory leak*/
        fclose(DataFile);
        } /* if() */
      else
        {
   ...
   else
     {
       ...
       fprintf(stderr, "The program must take in a file and the wrong number of arguments were excuted\n");      
       return ErrorCode;
      /* WRITE YOUR CODE HERE    */
